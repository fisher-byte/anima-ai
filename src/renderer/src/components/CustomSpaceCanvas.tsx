/**
 * CustomSpaceCanvas — 用户自定义 Space 画布（通用参数化版本）
 *
 * 接受一个 CustomSpaceConfig，动态生成隔离的画布空间。
 * 无种子节点，用户从零开始对话。
 * 文件名格式：custom-{id}-nodes.json / custom-{id}-conversations.jsonl / custom-{id}-edges.json
 */
import {
...
const SPACE_COLOR: Record<SpaceColorKey, { dot: string; accent: string; hover: string }> = {
  indigo:  { dot: 'rgba(99,102,241,0.10)',   accent: '#6366F1', hover: 'rgba(99,102,241,0.14)' },
  violet:  { dot: 'rgba(139,92,246,0.10)',   accent: '#8B5CF6', hover: 'rgba(139,92,246,0.14)' },
  emerald: { dot: 'rgba(16,185,129,0.10)',   accent: '#10B981', hover: 'rgba(16,185,129,0.14)' },
  amber:   { dot: 'rgba(245,158,11,0.10)',   accent: '#F59E0B', hover: 'rgba(245,158,11,0.14)' },
  rose:    { dot: 'rgba(244,63,94,0.10)',    accent: '#F43F5E', hover: 'rgba(244,63,94,0.14)' },
  sky:     { dot: 'rgba(14,165,233,0.10)',   accent: '#0EA5E9', hover: 'rgba(14,165,233,0.14)' },
}
...
function CustomNodeCard({
  node, spaceId, accentColor, hoverShadowColor,
  onOpen, onDelete, onPositionChange, onDragEnd, scale
}: CustomNodeCardProps) {
  const { t } = useT()
  ...
  const elemId = `custom-${spaceId}-node-${node.id}`
  ...
  }, [node.id, elemId, scale, onPositionChange])
...
interface SimNode { id: string; x: number; y: number; category: string; vx: number; vy: number }
...
interface CustomSpaceCanvasProps {
  isOpen: boolean
  onClose: () => void
  config: CustomSpaceConfig
}

export function CustomSpaceCanvas({ isOpen, onClose, config }: CustomSpaceCanvasProps) {
  const { t } = useT()
  const colors = SPACE_COLOR[config.colorKey] ?? SPACE_COLOR.indigo

  const nodesFile = `custom-${config.id}-nodes.json`
  ...
  const openCustomSpaceMode = useCanvasStore(state => state.openCustomSpaceMode)
  const closeCustomSpaceMode = useCanvasStore(state => state.closeCustomSpaceMode)
  ...
  simTickRef.current = () => {
    ...
    if (simNodes.length > 1 && temp > 0) {
      ...
      for (const n of simNodes) {
        const el = document.getElementById(`custom-${config.id}-node-${n.id}`)
  ...
  useEffect(() => {
    ...
    ;(async () => {
      const [nodesRaw, edgesRaw] = await Promise.all([
        storageService.read(nodesFile),
        storageService.read(edgesFile),
      ])
      let loadedNodes: Node[] = []
      let loadedEdges: EdgeType[] = []
      try { if (nodesRaw) loadedNodes = JSON.parse(nodesRaw) } catch { /* empty */ }
      try { if (edgesRaw) loadedEdges = JSON.parse(edgesRaw) } catch { /* empty */ }
  ...
  }, [isOpen, nodesFile, edgesFile, kickSim])
  ...
  }, [isOpen, isModalOpen, nodesFile, kickSim])

  useEffect(() => {
    if (isOpen) openCustomSpaceMode(config.id)
    else closeCustomSpaceMode()
  }, [isOpen, config.id, openCustomSpaceMode, closeCustomSpaceMode])

  useEffect(() => {
    if (!isHistoryOpen) return
    ;(async () => {
      const raw = await storageService.read(convsFile)
      if (!raw) return
  ...
  }, [isHistoryOpen, convsFile, t.space.noContent])
  ...
  useEffect(() => {
    ...
    const initScale = 0.7
    const cx = initialNodesCenterRef.current.x; const cy = initialNodesCenterRef.current.y
    const initOffset = { x: vw / 2 - cx * initScale + vw, y: vh / 2 - cy * initScale + vh }
    ...
    const handleWheel = (e: WheelEvent) => {
      ...
      if (!wheelRafRef.current) {
        wheelRafRef.current = requestAnimationFrame(() => {
          ...
          pendingWheelDeltaRef.current = 0; wheelRafRef.current = null
        })
      }
      if (scaleTimerRef.current) clearTimeout(scaleTimerRef.current)
      scaleTimerRef.current = setTimeout(() => { setScaleDisplay(viewRef.current.scale); scaleTimerRef.current = null }, 120)
    }
  ...
  const startInertia = useCallback(() => {
    const damping = 0.94
    const step = () => {
      velocity.current.x *= damping; velocity.current.y *= damping
  ...
  const handleCanvasMouseDown = useCallback((e: React.MouseEvent) => {
    ...
    if (target === canvasRef.current || target.classList.contains(dotGridClass)) {
  ...
  const handleNodeDragEnd = useCallback((id: string, x: number, y: number) => {
    ...
    setNodes(prev => {
      ...
      storageService.write(nodesFile, JSON.stringify(updated)).catch(() => {})
      return updated
    })
  }, [nodesFile])
    ...
    <div className="fixed inset-0 z-[100] flex flex-col" style={{ backgroundColor: '#f8f8fa' }}>
      ...
      >
        ...
        <div className="flex-1 flex items-center justify-center gap-3 min-w-0">
          <div
            className="w-8 h-8 rounded-full flex items-center justify-center text-white font-semibold text-[11px] shrink-0"
            style={{ backgroundColor: colors.accent }}
          >
            {config.avatarInitials}
          </div>
          <div className="min-w-0">
            <div className="text-sm font-semibold text-gray-800 leading-tight">{config.name}</div>
      ...
      {/* ── Canvas area ── */}
      <div
        className={`absolute inset-0 overflow-hidden ${dotGridClass} cursor-grab`}
        style={{ top: 56 }}
        ref={canvasRef}
      ...
      >
        ...
        {nodesLoaded && nodes.length === 0 && (
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
            <p className="text-sm text-gray-400 italic">{t.space.customPlaceholder(config.name)}</p>
          </div>
        )}
      </div>
          ...
          <div className="w-full max-w-2xl">
            ...
            >
              <textarea
                ...
                placeholder={t.space.customPlaceholder(config.name)}
                rows={1}
      ...
      <style>{`
        .${dotGridClass} {
          ...
          background-image: radial-gradient(circle, ${colors.dot} 1px, transparent 1px);
          background-size: 28px 28px;
        }
        .${dotGridClass}.\\!cursor-grabbing { cursor: grabbing !important; }
      `}</style>
