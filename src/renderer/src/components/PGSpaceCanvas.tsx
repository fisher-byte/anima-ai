 ...
 *
 * 与 LennySpaceCanvas 结构完全对称，换了：
 * - 数据源：PG_SEED_NODES / PG_SEED_EDGES（from @shared/pgData）
 * - Storage 文件：pg-nodes.json / pg-conversations.jsonl / pg-edges.json
 * - 主题色：indigo/violet 渐变
 * - 系统 prompt：PG_SYSTEM_PROMPT
 *
 * 数据来源：anima-base / people/startup/paul-graham/
...
function PGNodeCard({ node, onOpen, onDelete, onPositionChange, onDragEnd, scale }: PGNodeCardProps) {
  ...
  useLayoutEffect(() => {
    const el = document.getElementById(`pg-node-${node.id}`)
    ...
    >
      <motion.div
        ...
        className={`w-52 rounded-2xl border overflow-hidden transition-all duration-200 ${
          isHovered
            ? 'shadow-[0_8px_32px_rgba(99,102,241,0.14)] border-indigo-100'
        ...
        <div className="p-5 pl-6 relative">
          ...
          <div className="flex items-center justify-between text-[10px] text-gray-400 font-medium">
            ...
            {isHovered && (
              ...
              >
                {node.conversationId ? 'View history →' : 'Ask PG →'}
              </motion.span>
            )}
          </div>
          <div
      ...
      {/* 悬浮删除按钮（仅非种子节点可删） */}
      {isHovered && node.nodeType === 'memory' && !node.conversationId?.startsWith('pg-seed-') && (
        <motion.button
...
export function PGSpaceCanvas({ isOpen, onClose }: PGSpaceCanvasProps) {
  ...
  simTickRef.current = () => {
    ...
    if (simNodes.length > 1 && temp > 0) {
      ...
      for (const n of simNodes) {
        const el = document.getElementById(`pg-node-${n.id}`)
  ...
  useEffect(() => {
    ...
    ;(async () => {
      const [nodesRaw, edgesRaw] = await Promise.all([
        storageService.read(STORAGE_FILES.PG_NODES),
        storageService.read(STORAGE_FILES.PG_EDGES),
      ])
      ...
      if (loadedNodes.length === 0) {
        loadedNodes = PG_SEED_NODES
        loadedEdges = PG_SEED_EDGES
        await Promise.all([
          storageService.write(STORAGE_FILES.PG_NODES, JSON.stringify(loadedNodes)),
          storageService.write(STORAGE_FILES.PG_EDGES, JSON.stringify(loadedEdges)),
        ])
      }
  ...
  useEffect(() => {
    ...
    ;(async () => {
      const nodesRaw = await storageService.read(STORAGE_FILES.PG_NODES)
      if (!nodesRaw) return
      try {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      const raw = await storageService.read(STORAGE_FILES.PG_CONVERSATIONS)
      if (!raw) return
  ...
  const handleCanvasMouseDown = useCallback((e: React.MouseEvent) => {
    ...
    if (target === canvasRef.current || target.classList.contains('pg-dot-grid')) {
  ...
  const handleNodeDragEnd = useCallback((id: string, x: number, y: number) => {
    ...
    setNodes(prev => {
      ...
      storageService.write(STORAGE_FILES.PG_NODES, JSON.stringify(updated)).catch(() => {})
      return updated
    })
  }, [])
  ...
  const handleNodeOpen = useCallback((node: Node) => {
    const isSeedNode = !node.conversationId || node.conversationId.startsWith('pg-seed-')
    if (!isSeedNode) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex flex-col" style={{ backgroundColor: '#f8f8fa' }}>
      ...
      {/* ── Canvas area ── */}
      <div
        className="absolute inset-0 overflow-hidden pg-dot-grid cursor-grab"
        style={{ top: 56 }}
        ref={canvasRef}
      ...
      <style>{`
        ...
        .pg-dot-grid.\\!cursor-grabbing { cursor: grabbing !important; }
      `}</style>
