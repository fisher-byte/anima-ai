 ...
 * - 数据源：ZHANG_SEED_NODES / ZHANG_SEED_EDGES（from @shared/zhangData）
 * - Storage 文件：zhang-nodes.json / zhang-conversations.jsonl / zhang-edges.json
 * - 主题色：blue/cyan 渐变
 * - 系统 prompt：ZHANG_SYSTEM_PROMPT
 *
 * 数据来源：anima-base / people/product/zhang-xiaolong/
...
function ZhangNodeCard({ node, onOpen, onDelete, onPositionChange, onDragEnd, scale }: ZhangNodeCardProps) {
  const { t } = useT()
  ...
  useLayoutEffect(() => {
    const el = document.getElementById(`zhang-node-${node.id}`)
    ...
    >
      <motion.div
        ...
        className={`w-52 rounded-2xl border overflow-hidden transition-all duration-200 ${
          isHovered
            ? 'shadow-[0_8px_32px_rgba(59,130,246,0.14)] border-blue-100'
        ...
        <div className="p-5 pl-6 relative">
          ...
          <div className="flex items-center justify-between text-[10px] text-gray-400 font-medium">
            ...
            {isHovered && (
              ...
              >
                {node.conversationId ? 'View history →' : 'Ask Zhang →'}
              </motion.span>
            )}
          </div>
          <div
      ...
      {/* 悬浮删除按钮（仅非种子节点可删） */}
      {isHovered && node.nodeType === 'memory' && !node.conversationId?.startsWith('zhang-seed-') && (
        <motion.button
...
export function ZhangSpaceCanvas({ isOpen, onClose }: ZhangSpaceCanvasProps) {
  ...
  const openZhangMode = useCanvasStore(state => state.openZhangMode)
  const closeZhangMode = useCanvasStore(state => state.closeZhangMode)
  ...
  simTickRef.current = () => {
    ...
    if (simNodes.length > 1 && temp > 0) {
      ...
      for (const n of simNodes) {
        const el = document.getElementById(`zhang-node-${n.id}`)
  ...
  useEffect(() => {
    ...
    ;(async () => {
      const [nodesRaw, edgesRaw] = await Promise.all([
        storageService.read(STORAGE_FILES.ZHANG_NODES),
        storageService.read(STORAGE_FILES.ZHANG_EDGES),
      ])
      ...
      if (loadedNodes.length === 0) {
        loadedNodes = ZHANG_SEED_NODES
        loadedEdges = ZHANG_SEED_EDGES
        await Promise.all([
          storageService.write(STORAGE_FILES.ZHANG_NODES, JSON.stringify(loadedNodes)),
          storageService.write(STORAGE_FILES.ZHANG_EDGES, JSON.stringify(loadedEdges)),
        ])
      }
  ...
  useEffect(() => {
    ...
    ;(async () => {
      const nodesRaw = await storageService.read(STORAGE_FILES.ZHANG_NODES)
      if (!nodesRaw) return
      try {
  ...
  useEffect(() => {
    if (isOpen) openZhangMode()
    else closeZhangMode()
  }, [isOpen, openZhangMode, closeZhangMode])

  useEffect(() => {
    if (!isHistoryOpen) return
    ;(async () => {
      const raw = await storageService.read(STORAGE_FILES.ZHANG_CONVERSATIONS)
      if (!raw) return
  ...
  const handleCanvasMouseDown = useCallback((e: React.MouseEvent) => {
    ...
    if (target === canvasRef.current || target.classList.contains('zhang-dot-grid')) {
  ...
  const handleNodeDragEnd = useCallback((id: string, x: number, y: number) => {
    ...
    setNodes(prev => {
      ...
      storageService.write(STORAGE_FILES.ZHANG_NODES, JSON.stringify(updated)).catch(() => {})
      return updated
    })
  }, [])
  ...
  const handleNodeOpen = useCallback((node: Node) => {
    const isSeedNode = !node.conversationId || node.conversationId.startsWith('zhang-seed-')
    if (!isSeedNode) {
    ...
    <div className="fixed inset-0 z-[100] flex flex-col" style={{ backgroundColor: '#f8f8fa' }}>
      ...
      >
        ...
        <div className="flex-1 flex items-center justify-center gap-3 min-w-0">
          <div className="w-8 h-8 rounded-full bg-blue-600 flex items-center justify-center text-white font-semibold text-[11px] shrink-0">
            张
          </div>
          <div className="min-w-0">
            <div className="text-sm font-semibold text-gray-800 leading-tight">张小龙</div>
      ...
      {/* ── Canvas area ── */}
      <div
        className="absolute inset-0 overflow-hidden zhang-dot-grid cursor-grab"
        style={{ top: 56 }}
        ref={canvasRef}
      ...
      >
        ...
        {nodesLoaded && nodes.length === 0 && (
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
            <p className="text-sm text-gray-400 italic">{t.space.loading('张小龙')}</p>
          </div>
        )}
      </div>
        ...
        {isHistoryOpen && (
          ...
          >
            <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
              <span className="text-sm font-semibold text-gray-700">{t.space.historyTitle('张小龙')}</span>
          ...
          <div className="w-full max-w-2xl">
            ...
            >
              <textarea
                ...
                placeholder={t.space.zhangPlaceholder}
                rows={1}
      ...
      <style>{`
        ...
        .zhang-dot-grid.\\!cursor-grabbing { cursor: grabbing !important; }
      `}</style>
