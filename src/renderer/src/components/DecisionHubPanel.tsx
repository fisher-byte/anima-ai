...
export function DecisionHubPanel({
  ...
  return (
    <motion.div
      ...
      className="fixed top-4 bottom-4 right-4 z-40 flex w-[420px] flex-col overflow-hidden rounded-[32px] border border-white/70 bg-white/90 shadow-[0_28px_90px_rgba(15,23,42,0.16)] backdrop-blur-xl"
    >
      <div className="border-b border-gray-100/80 px-6 py-5">
        <button
          onClick={onClose}
          className="absolute right-5 top-5 rounded-full p-2 text-gray-400 transition-colors hover:bg-gray-100 hover:text-gray-600"
        >
          <X className="h-4 w-4" />
        </button>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <h2 className="mt-1 text-2xl font-bold tracking-tight text-gray-900">决策追踪</h2>
          </div>
        </div>
        ...
        <div className="mt-4 grid grid-cols-3 gap-3">
          ...
          <div className="rounded-2xl border border-blue-100 bg-blue-50/70 px-4 py-3">
            <div className="text-[11px] font-medium text-blue-800">进行中</div>
            <div className="mt-1 text-2xl font-semibold text-blue-950">{activeItems.length}</div>
          </div>
      ...
      <div className="flex-1 space-y-5 overflow-y-auto px-5 py-5">
        {groups.map((group) => (
          <section key={group.title}>
            ...
            <p className="mb-3 text-[12px] leading-5 text-gray-500">{group.subtitle}</p>

            {group.items.length === 0 ? (
              <div className="rounded-2xl border border-dashed border-gray-200 bg-gray-50/70 px-4 py-4 text-[12px] leading-5 text-gray-400">
                {group.title === '验证台账'
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          ...
                          <div className="mt-1 line-clamp-2 text-[15px] font-semibold leading-6 text-gray-900">
                            {item.title}
                          </div>
                        </div>
                      ...
                      <div className="mt-2 text-[13px] leading-6 text-gray-600">
                        {item.decisionRecord.recommendationSummary}
                      </div>
                      ...
                      {item.notes && (
                        <div className="mt-3 rounded-2xl bg-gray-50 px-3 py-2 text-[12px] leading-5 text-gray-600">
                          {item.notes}
                        </div>
                      )}
                    </button>
