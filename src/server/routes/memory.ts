...
用户说：${cleanUserMessage.slice(0, 500)}
${assistantMessage ? `\n（AI回复仅供理解上下文，不作为提取来源）` : ''}

从用户发言中提取有价值的记忆条目。要求每条记忆**包含场景/状态信息**，不要只写结论：
- 个人信息：职业、背景、所在地等（如"用户是做 AI 产品的独立开发者"）
- 当前在做什么：结合场景描述（如"用户正在做 GEO 小工具，处于探索初期，想摸索变现路径"，而不是"用户目标是变现"）
- 项目/想法状态：包含阶段和背景（如"用户打算清明节后看市场机会，考虑是否在某赛道切入"，而不是"用户计划清明后看机会"）
- 观点偏好：对某事有明确看法时记录（如"用户认为 AI 工具要先找到高频刚需场景，不认可纯内容方向"）
- 目标计划：含时间节点和前提（如"用户短期目标是先做出一个有真实用户的 MVP，而非追求完整产品"）

规则：
- 每条不超过 35 字，但必须包含足够的上下文信息
- 只提取用户亲口说出来的，不要推测或扩展
- 纯闲聊/寒暄/一两个字的短回复 → 返回空数组
- 只返回 JSON，不要其他文字
...
memoryRoutes.post('/sync-lenny-conv', async (c) => {
  const db = userDb(c)
  const { conversationId, userMessage, assistantMessage, source, decisionTrace, decisionRecord } = await c.req.json<{
