...
**会话持久化（v0.5.43+）**：Space 内每轮回答结束后 `AnswerModal.autoSaveIfNeeded` 同样 `appendConversation` 至对应 `*-conversations.jsonl`；刷新前 `beforeunload` 按 `getConversationsPersistFilename()` 调用 `POST /api/storage/{filename}/append`。从画布「历史」或节点打开对话时，`openModalById(id, undefined, sourceHint)` 恢复 Space 模式，避免误读主空间 `conversations.jsonl`。对话窗顶部徽章区分灵思决策 / 普通对话与模型思考过程。
...
```
Layer 1 — 节点级（v0.5.44 起参数略调，并增加近距软斥力）
  ├── 全局斥力：NODE_REPEL≈15000，距离 <640px 生效
  ├── 近距软斥力：中心距 <235px 时额外推开，减轻卡片叠在同一点
  ├── 同类引力弹簧：SAME_ATTRACT≈0.00125，理想间距约 300px
  ├── 异类斥力：DIFF_REPEL≈140
  ├── 连线弹簧：EDGE_SPRING=0.0025，理想长度 300px
  └── 全局中心引力：CENTER_GRAVITY≈0.000045（防飘出，略降以减轻向心塌缩）
...
- `temp=0` 时布局速度归零；**主画布首次出现记忆节点须 `kick()`**，否则斥力/弹簧不积分位移、节点会堆叠（v0.5.44 起与「节点数增加」统一 `kick()`）。kick 后每帧仍经阻尼，避免速度爆发。
