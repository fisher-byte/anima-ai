...
| `npm test` | 运行所有测试（单元 + 集成，当前 569 个用例，25 个文件） |
| `npm run test:watch` | 监听模式（开发时用） |
| `npm run typecheck` | TypeScript 类型检查 |
| `npm run lint` | ESLint 检查 |
| `npm run lingsi:evaluate` | 跑 LingSi 15 题 `normal vs decision` 对照评测；支持 `LINGSI_EVAL_CASE` / `LINGSI_EVAL_TIMEOUT_MS` |
