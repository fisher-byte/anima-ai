...
**对话 UI**:
- [x] **模型标签**: 从 header 移至输出区左上角轻量展示，与之前设计一致。
- [x] **复制/编辑/重试**: 参考 ChatGPT，统一放在用户与 AI 气泡的右下方，hover 显示。
- [x] **输出区收窄**: 内容区与输入区改为 `max-w-xl`，阅读更集中。
- [x] **思考区**: 默认折叠（有正文时首帧即折叠），展开样式增强（左边框 + 浅底）。
