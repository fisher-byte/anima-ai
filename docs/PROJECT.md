...
> 最后更新：2026-03-18 | 当前版本：v0.5.24

---

## 当前冲刺（v0.5.24 已完成）

*本轮收口的是“产品状态包 fallback”展示缺口：当 `@persona` 的决策回答没有命中具体案例时，决策轨迹改成能让用户读懂的项目状态依据，而不是一串原始 docs 路径。*

| # | 任务 | 状态 | 说明 |
|---|------|------|------|
| 1 | 产品状态 fallback 去路径化 | 已完成 | `docs/*.md` 原始路径不再直接暴露给用户，统一翻译成“当前冲刺 / 路线图 / 变更记录 / 评测基线”等标签 |
| 2 | 0 命中时的轨迹空白修复 | 已完成 | 决策轨迹弹层在 `0 Decision Unit / 0 source` 时不再出现空白左栏，改成明确说明“本次主要依据当前项目状态判断” |
| 3 | 轨迹文案收敛 | 已完成 | 轨迹卡片从“决策单元与真实来源片段”收敛为更符合 fallback 语义的说明 |
| 4 | 文档与发布收口 | 已完成 | 已完成 docs 同步、review、`npm run typecheck`、`npm test`、`npm run build`、`npm run test:e2e`，并已部署到线上 |
