...
| # | 任务 | 状态 | 说明 |
|---|------|------|------|
| 1 | 极简视觉重设计 | 🔄 进行中 | 去掉逻辑边六色 + 语义边紫线，统一极淡黑色；节点改纯白 + 左侧 accent 色条 |
| 2 | P1 技术债：静默吞错改善 | ⏳ 待做 | AI/存储失败时给用户可感知的 toast 提示 |
| 3 | P1 技术债：节点虚拟化 | ⏳ 待做 | 80+ 节点时只渲染视口内节点，空间哈希实现 |
...
| 任务 | 文件 | 说明 |
|------|------|------|
| 静默吞错改善 | `canvasStore.ts`, `InputBox.tsx` | `startConversation` catch → toast；存储失败 → console.warn（不打断） |
| 节点虚拟化 | `Canvas.tsx` | 空间哈希 `Map<gridKey, nodeId[]>`，按 offset/scale 计算可见集，80+ 节点时生效 |
...
| 版本 | 日期 | 核心内容 |
|------|------|----------|
| v0.2.54 | 2026-03-07 | E2E token 隔离（P0）+ MemoryLines 颜色映射修复 |
...
| 问题 | 优先级 | 状态 |
|------|--------|------|
| 节点无虚拟化（80+ 帧率下降） | P1 | ⏳ 待做 |
| 静默吞错（AI/存储失败无感知） | P1 | ⏳ 待做 |
| prompt.ts 僵尸文件 | P2 | ⏳ 待做 |
