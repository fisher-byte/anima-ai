...
| 8 | 回访提醒 + 决策追踪面板 | 已完成 | 主页新增 `决策追踪` 入口，按“今天该回访 / 进行中 / 验证台账”分组，支持继续打开原决策对话 |
| 9 | 验证台账与结果备注 | 已完成 | 回访结果现在支持写备注，已复盘决策会自动进入验证台账，作为后续沉淀 unit / eval 的原材料 |
| 10 | 真实用户验证台账（真实数据） | 待做 | 为首批非团队用户记录“问题 -> 建议 -> 采纳 -> 结果”，避免继续用系统建设替代用户验证 |
| 11 | 文档与发布收口 | 已完成 | 已完成 typecheck / test / build / e2e、docs 同步、review、GitHub 备份与部署验证 |
