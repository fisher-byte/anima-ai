...
export function Canvas() {
  ...
  const heavyLayerRef = useRef<HTMLDivElement>(null)
  ...
  const setZooming = useCallback((zooming: boolean) => {
    if (isZoomingRef.current === zooming) return
    isZoomingRef.current = zooming
    setIsZoomingState(zooming)
    if (heavyLayerRef.current) {
      heavyLayerRef.current.style.visibility = zooming ? 'hidden' : 'visible'
    }
  }, [])
  ...
  }, [applyTransform, setOffset, setScale, setZooming])
    ...
    <>
      ...
      >
        ...
        >
          ...
          >
          {!isZooming && (
            <div ref={heavyLayerRef} style={{ position: 'absolute', inset: 0 }}>
              {/* 连线渲染 (SVG层) */}
              <svg
