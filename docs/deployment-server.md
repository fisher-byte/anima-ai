...
**最后更新**: 2026-03-21
**应用版本**: v0.5.45（生产目录 `/opt/evocanvas`，PM2 进程名 `evocanvas`）
...
>
> **多用户/多 token**：若需要多个访问令牌（和本地 .env 里一致），必须设置 `ACCESS_TOKENS=token1,token2,token3`（逗号分隔，不要换行）；服务端会对 token 做 trim，首尾空格不影响校验。

### 部署与 .env 同步

- 一键部署脚本 `docs/scripts/deploy.sh` 打包时**不包含** `.env`（`--exclude='.env'`），因此服务器上的 `.env` 不会随代码更新被覆盖。
- 若本地配置了多 token（`ACCESS_TOKENS`），部署后需让服务器与本地一致，否则多 token 登录会 403。可选：
  - **部署时同步**：`SYNC_ENV=1 bash docs/scripts/deploy.sh`（在项目根目录执行），会在部署完成后把本地 `.env` 上传到服务器并执行 `pm2 restart evocanvas`。
  - **手动同步**：`scp .env root@101.32.215.209:/opt/evocanvas/.env` 后 SSH 上服务器执行 `pm2 restart evocanvas`。
- 注意：脚本中 `REMOTE_DIR` 为 `/opt/evocanvas`，若你实际使用 `/opt/anima`，请修改脚本或上述路径。
