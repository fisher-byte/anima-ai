...
const SOURCE_SPECS: SourceSpec[] = [
  ...
  {
    id: 'src-lenny-ai-evals',
    personaId: 'lenny',
    type: 'framework',
    person: 'Hamel Husain & Shreya Shankar',
    sourcePath: 'people/product/lenny-rachitsky/frameworks/ai-evals-framework.md',
    label: 'AI Evals Framework',
    title: 'AI Evals Framework',
    url: 'https://www.youtube.com/watch?v=BsWxPI9UM4c',
    publishedAt: '2025-09-25',
    notes: 'Open coding, eval rubric, representative data, regression safety for AI products',
    evidenceLevel: 'A',
    mustInclude: ['It\'s the highest ROI activity you can engage in.', 'Quality > Quantity. 50 well-chosen examples > 1000 random ones.', 'One person can build initial evals (avoid committee paralysis)'],
  },
  {
    id: 'src-lenny-product-velocity',
    personaId: 'lenny',
    type: 'framework',
    person: 'Lenny Rachitsky',
    sourcePath: 'people/product/lenny-rachitsky/frameworks/product-velocity-framework.md',
    label: 'Product Velocity Framework',
    title: 'Product Velocity Framework',
    publishedAt: '2026-03-18',
    notes: 'Decision speed hierarchy, time-boxed debates, built-in analytics, variable scope with fixed time',
    evidenceLevel: 'B',
    mustInclude: ['Decision Speed Hierarchy:', '- **Time-box debates**: 30min → decide or escalate', '- **Built-in analytics**: Instrument before launch'],
  },
  {
    id: 'src-zhang-product-philosophy-2012',
    personaId: 'zhang',
    type: 'article',
    person: 'Zhang Xiaolong',
    sourcePath: 'people/product/zhang-xiaolong/articles/2012-wechat-product-philosophy-speech.md',
    label: 'WeChat Product Philosophy Speech (2012)',
    title: '微信背后的产品观：简单之美与人性洞察',
    publishedAt: '2012-06-15',
    notes: 'No explanatory text, primal interactions, simple rules drive complex evolution',
    evidenceLevel: 'A',
    mustInclude: ['一旦一个功能需要文字解释,这个功能的设计已经失败了。', '如果要经过学习,它就不好了。', '只有简单的规则才可以演化出非常复杂的事件。'],
  },
  {
    id: 'src-zhang-wxg-leadership-2016',
    personaId: 'zhang',
    type: 'article',
    person: 'Zhang Xiaolong',
    sourcePath: 'people/product/zhang-xiaolong/talks/talk-wxg-leadership-2016.md',
    label: 'WXG Leadership Talk (2016)',
    title: 'KPI是副产品，警惕复杂流程——WXG领导力大会内部分享',
    url: 'https://www.digitaling.com/articles/31588.html',
    publishedAt: '2016-10-30',
    notes: 'KPI as byproduct, use-and-go products, 150-person team limits, small team agility',
    evidenceLevel: 'A',
    mustInclude: ['### KPI应该是副产品', '**"好的产品是让用户用完即走"**', '**"一个10人敏捷团队的产出远超之前几十人的努力"**'],
  },
]
...
const UNIT_SEEDS: UnitSeed[] = [
  ...
  {
    id: 'lenny-open-code-real-failures-before-eval-rubric',
    personaId: 'lenny',
    title: '做 AI 质量判断时，先看真实失败样本，再写 rubric',
    summary: 'AI 产品质量问题不要先拍脑袋写 rubric。先抽一批真实样本做 open coding，看失败模式自然浮现，再转成可执行的 eval。',
    scenario: '团队在做 AI 产品优化，但总觉得 prompt 改来改去像猜谜，没有统一质量标尺。',
    goal: '让 AI 质量迭代从主观 vibe check 变成可复用的评测体系。',
    constraints: ['失败样本分散', '质量标准主观', '团队容易陷入口水战'],
    tags: ['ai', 'evals', 'quality', 'rubric'],
    triggerKeywords: ['AI 质量', '评测', 'eval', 'rubric', 'LLM judge', 'prompt 调优', '提示词优化'],
    preferredPath: '先抽取真实样本做 open coding，再定义 rubric 和自动化 eval，不要直接从空白表格开始。',
    antiPatterns: ['没有真实样本就先写评测标准', '只凭主观感觉改 prompt', '委员会一起定义 rubric 导致迟迟不落地'],
    reasoningSteps: ['先收集代表性真实样本', '由最懂产品质量的人做 open coding', '再把观察转成 eval rubric'],
    reasons: [
      'AI 质量的关键不是先有完美 rubric，而是先看真实失败长什么样。',
      '让分类维度从样本里长出来，比先预设维度更接近真实产品问题。',
      '先把 eval 建起来，后续每次 prompt 或模型变更才有回归保护。',
    ],
    followUpQuestions: [
      '你们最近 50-100 个真实失败样本里，最常见的 3 类问题是什么？',
      '现在是谁最懂“什么叫好输出”，这个人是否亲自做过 open coding？',
    ],
    nextActions: [
      '从生产环境抽 50-100 个近期样本，覆盖常见场景、边界场景和已知失败。',
      '指定 1 个最懂产品质量的人做 open coding，不用委员会并行审。',
      '把归纳出的失败模式转成 rubric，再接入 heuristic 或 LLM-as-judge eval。',
    ],
    evidenceLevel: 'A',
    evidenceRefs: [
      {
        sourceId: 'src-lenny-ai-evals',
        excerpt: '**Key Insight**: "Quality > Quantity. 50 well-chosen examples > 1000 random ones."',
      },
      {
        sourceId: 'src-lenny-ai-evals',
        excerpt: '- One person can build initial evals (avoid committee paralysis)',
      },
    ],
    confidence: 0.88,
  },
  {
    id: 'lenny-evals-before-more-model-swaps',
    personaId: 'lenny',
    title: 'AI 产品先把 eval 体系搭起来，再频繁换模型和 prompt',
    summary: '如果没有 eval，模型切换、prompt 修改和策略调整都只是在碰运气。先把评测闭环立住，迭代才会越改越准。',
    scenario: '团队准备频繁换模型、调 prompt 或上更多 agent 策略，但缺少稳定验证机制。',
    goal: '减少盲目迭代和隐藏回归。',
    constraints: ['改动频繁', '问题主观', '线上回归难定位'],
    tags: ['ai', 'evals', 'iteration', 'regression'],
    triggerKeywords: ['换模型', '改 prompt', '回归', 'AI 迭代', '质量回退', '评测体系'],
    preferredPath: '先建 eval，再让每次模型或 prompt 迭代都跑一遍回归，不要先大改后手工抽查。',
    antiPatterns: ['没有基线就频繁换模型', '上线前只靠手感抽查', '修一个问题又破坏另一个问题'],
    reasoningSteps: ['先定义当前质量基线', '再把历史失败加入回归集', '让每次变更都经过同一套 eval'],
    reasons: [
      'AI 产品最危险的不是一次失败，而是每次修补都可能引入新回归。',
      '只有评测基线稳定，团队才知道改变到底让产品更好还是更糟。',
      '回归集会把过去踩过的坑沉淀成系统记忆，避免重复返工。',
    ],
    followUpQuestions: [
      '你们现在每次换模型后，靠什么判断它真比原来更好？',
      '历史上修过的失败案例，有没有进入固定回归集？',
    ],
    nextActions: [
      '先把近期最关键的失败案例沉淀成回归测试集。',
      '在每次模型、prompt 或 agent 策略变更后强制跑 eval。',
      '把“质量是否改善”从口头判断改成固定指标和失败类型对比。',
    ],
    evidenceLevel: 'A',
    evidenceRefs: [{
      sourceId: 'src-lenny-ai-evals',
      excerpt: '> "To build great AI products, you need to be really good at building evals. It\'s the highest ROI activity you can engage in." — Hamel Husain',
    }],
    confidence: 0.87,
  },
  {
    id: 'lenny-ship-on-fixed-time-variable-scope',
    personaId: 'lenny',
    title: '速度不够时，先固定时间，再压缩范围',
    summary: '当团队总说项目太慢，不要默认继续拉长排期。更稳的做法是固定交付时间，把范围压到最小可验证切片。',
    scenario: '团队推进新功能或新项目时，经常因为范围膨胀和完美主义导致延迟。',
    goal: '提高交付速度，同时保留学习节奏。',
    constraints: ['资源有限', '需求膨胀', '团队追求完整性'],
    tags: ['velocity', 'scope', 'shipping'],
    triggerKeywords: ['推进太慢', '上线太慢', '范围太大', '交付速度', '切片', 'ship fast'],
    preferredPath: '固定时间盒，按最小端到端切片交付；不要用无上限扩排期来保护完整性。',
    antiPatterns: ['范围和时间一起膨胀', '小功能也按大项目推进', '没有 analytics 就先上线'],
    reasoningSteps: ['先定义本轮必须验证的核心价值', '再按时间盒反推范围', '上线前确保埋点和回滚方案齐备'],
    reasons: [
      '真正的速度不是同时追求全量范围，而是更快拿到下一轮学习。',
      '固定时间、变量范围比固定范围、变量时间更能保护团队节奏。',
      '内建 analytics 和 rollout 规则，能让快迭代不演变成盲上线。',
    ],
    followUpQuestions: [
      '这次交付里，哪 20% 的范围最能验证核心价值？',
      '如果上线时间不变，哪些部分其实可以放到第二轮？',
    ],
    nextActions: [
      '把当前项目切成 1 个端到端最小版本和后续增强版本。',
      '固定这轮发布时间，只保留最小可验证范围。',
      '在上线前补齐埋点、feature flag 和 rollback plan。',
    ],
    evidenceLevel: 'B',
    evidenceRefs: [
      {
        sourceId: 'src-lenny-product-velocity',
        excerpt: '- **Time-box debates**: 30min → decide or escalate',
      },
      {
        sourceId: 'src-lenny-product-velocity',
        excerpt: '- **Built-in analytics**: Instrument before launch',
      },
    ],
    confidence: 0.84,
  },
  ...
  {
    id: 'zhang-no-tips-means-the-interaction-is-natural',
    personaId: 'zhang',
    title: '一个功能如果必须靠解释才能用，通常设计已经错了',
    summary: '交互设计的目标不是把说明写得更清楚，而是让用户天然知道该怎么做。需要大量 tips 的功能，通常说明规则和动作还不够自然。',
    scenario: '团队在争论一个新功能是否应该通过 tooltip、教程或 onboarding 解释给用户。',
    goal: '判断该继续补说明，还是回头重做交互本身。',
    constraints: ['团队已经做了不少说明文案', '担心重做交互成本高'],
    tags: ['ux', 'interaction', 'simplicity'],
    triggerKeywords: ['教程', 'tip', '引导文案', '看不懂', '不会用', '交互设计'],
    preferredPath: '优先重做交互和动作本身，让用户不看说明也知道怎么用；不要先补更多解释。',
    antiPatterns: ['功能看不懂就继续加 tooltip', '复杂流程靠新手教程硬教', '把教育成本都压给用户'],
    reasoningSteps: ['先判断用户是否能自然猜到动作', '再检查是否需要文字解释', '如果需要就回头简化交互'],
    reasons: [
      '用户需要先学习才能操作，往往意味着动作本身不够贴近直觉。',
      '说明文案只能缓解问题，不能替代自然的交互设计。',
      '真正顺手的功能会让用户下意识地产生正确动作，而不是先看教程。',
    ],
    followUpQuestions: [
      '不看任何文案，新用户能否在 5 秒内猜到这个功能怎么触发？',
      '如果删掉 tooltip，这个功能是否还成立？',
    ],
    nextActions: [
      '把现有 tooltip 和教程文案临时拿掉，观察用户是否还能完成核心动作。',
      '优先重做触发动作和反馈，而不是继续堆说明。',
      '让一个完全不了解背景的人做首次测试，记录他是否能自然完成。',
    ],
    evidenceLevel: 'A',
    evidenceRefs: [
      {
        sourceId: 'src-zhang-product-philosophy-2012',
        excerpt: '我们会避免在界面里面出现任何的文字解释。**一旦一个功能需要文字解释,这个功能的设计已经失败了。**',
      },
      {
        sourceId: 'src-zhang-product-philosophy-2012',
        excerpt: '我们买一个iPhone或者iPad给一个四岁小孩子用都会用,四岁的小孩体现的是原始或者简单,那么它是体验好的。如果要经过学习,它就不好了。',
      },
    ],
    confidence: 0.9,
  },
  {
    id: 'zhang-kpi-is-a-byproduct-not-a-driver',
    personaId: 'zhang',
    title: 'KPI 应该是结果，不应该反过来驱动产品选择',
    summary: '当团队开始用数字漂亮与否来定义方向时，方法会很快走形。更稳的方式是先判断用户价值是否成立，让 KPI 成为副产品。',
    scenario: '团队在路线图、运营或商业化选择上被 KPI 压着走，开始为了数字而设计流程。',
    goal: '避免产品为短期指标服务，而不是为用户价值服务。',
    constraints: ['公司有硬指标', '业务压力会不断要求数字证明'],
    tags: ['kpi', 'strategy', 'product-value'],
    triggerKeywords: ['KPI', '指标导向', '数字好看', '活跃目标', '年 PV', '数据驱动'],
    preferredPath: '先定义对用户真正有价值的目标，再看 KPI 是否自然随之改善；不要先为了数字设计动作。',
    antiPatterns: ['为了指标做反用户动作', '先找数字目标再倒推出产品方案', '用大数字掩盖真实日常价值'],
    reasoningSteps: ['先确认用户价值', '再定义能反映价值的指标', '最后拒绝只为数字服务的动作'],
    reasons: [
      '数字可以被包装，但用户价值一旦偏了，产品会很快失真。',
      '当 KPI 变成直接驱动力，团队会不断设计对数字友好但对用户不友好的路径。',
      '让 KPI 成为副产品，反而更容易得到长期更稳的结果。',
    ],
    followUpQuestions: [
      '这个目标是在证明用户价值，还是只是在证明一个好看的数字？',
      '如果去掉 KPI 压力，你们还会做这件事吗？',
    ],
    nextActions: [
      '把当前目标拆成“用户价值目标”和“KPI 结果指标”两层。',
      '逐条筛掉那些只会抬数字、不会提升用户价值的方案。',
      '在评审里强制要求先说明用户价值，再说明数字影响。',
    ],
    evidenceLevel: 'A',
    evidenceRefs: [{
      sourceId: 'src-zhang-wxg-leadership-2016',
      excerpt: '在 QQ 邮箱开始快速发展的时候，我记得在内部还过一次分享，当时我说了一句话，叫做**我们达到了 KPI 是我们产品的副产品**。所谓副产品就是说，我们真的把这个东西做好以后我们的 KPI 自然就达到了。早期的微信团队也一直是围绕这样一个思路在工作。',
    }],
    confidence: 0.89,
  },
  {
    id: 'zhang-small-team-speed-beats-large-process',
    personaId: 'zhang',
    title: '窗口期和创新期里，小团队速度比大流程更重要',
    summary: '一件事如果要靠跨大团队流程推进，往往已经在失去敏捷性。创新窗口里更稳的选择，是缩成小团队、快速迭代、快速验证。',
    scenario: '组织规模变大后，功能和项目推进越来越慢，任何改动都要排很久。',
    goal: '在组织变大时保住关键创新和交付速度。',
    constraints: ['团队规模大', '跨部门协作复杂', '流程已经固化'],
    tags: ['team', 'agility', 'execution'],
    triggerKeywords: ['小团队', '敏捷', '推进太慢', '流程太重', '组织变大', '迭代速度'],
    preferredPath: '对窗口期任务收缩为小而全的直接负责团队，先恢复快速迭代，不要继续加流程求稳。',
    antiPatterns: ['组织越大越靠更多流程解决速度问题', '小改动也必须排长周期', '把所有事情都塞进统一大团队排期'],
    reasoningSteps: ['先识别是否是组织复杂度拖慢了速度', '再缩小负责团队边界', '用快速验证替代长周期证明'],
    reasons: [
      '大团队的默认问题不是能力不足，而是沟通和排期成本不断累积。',
      '创新窗口里，先恢复一个小团队的作战速度，比继续叠流程更重要。',
      '小团队能更快地犯错、修正和再前进，学习速度通常远高于大组织流程。',
    ],
    followUpQuestions: [
      '这件事真正需要多少人直接协作，还是只是被现有组织结构绑住了？',
      '现在最慢的部分，是能力问题，还是预约、评审和排队问题？',
    ],
    nextActions: [
      '把当前最关键项目拆成一个小而全的直接负责小组，减少跨层级依赖。',
      '给这支小组设置短迭代验证节奏，而不是长周期大排期。',
      '复盘每次延误到底来自功能难度，还是来自组织沟通链条。',
    ],
    evidenceLevel: 'A',
    evidenceRefs: [
      {
        sourceId: 'src-zhang-wxg-leadership-2016',
        excerpt: '**"一个10人敏捷团队的产出远超之前几十人的努力"**',
      },
      {
        sourceId: 'src-zhang-wxg-leadership-2016',
        excerpt: '我担忧的是，我们作为一个上千人的组织，如果当成 10 个 150 人团队的话，我认为它会有非常高的创造力，如果当成整体 1500 人，我特别担心它在创造的能力上会不会反而有一些衰退。',
      },
    ],
    confidence: 0.88,
  },
